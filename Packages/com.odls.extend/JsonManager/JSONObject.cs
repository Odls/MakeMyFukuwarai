...
public class JSONObject : System.IDisposable {
	[System.Flags]
	public enum ToStringMode {
		Original,
		ConvertEscapeCharacters = 0b0001,
		DecodeUnicode = 0b0010,
		DecodeUnicodeAndEscapeCharacters = ConvertEscapeCharacters | DecodeUnicode,

	}


#if POOLING
	public static readonly Queue<JSONObject> releaseQueue = new Queue<JSONObject>();
#endif
	...
	bool TryGetObject(string key,out JSONObject json) {
		json = null;
		json=GetField(key);
		return json!=null;
	}
	public JSONObject TryGetObject(string key,bool logError = true) {
		nowTryGetKey = key;
		JSONObject _json = null;
		if (TryGetObject (key, out _json)) {
			if(_json.IsNull){
				Debug.LogError ("[" + nowTryGetKey + "] in Json is A Null Obj");
			}
			nowTryGetKey = "";
			return _json;
		} else {
			if(logError){
				Debug.LogError("NO [" + key + "] in Json : " + Print());
			}
			nowTryGetKey = "";
			return nullJO;
		}
	...
	public T TryGetObjectToData<T>(string key,bool logError = true) where T : IJsonData, new(){
		JSONObject _json = TryGetObject(key,logError);
		T _tempData = new T();
		_tempData.SetData(_json);
		return _tempData;
	}

	public List<JSONObject> TryGetList(string key,bool logError = true) {
		nowTryGetKey = key;
		JSONObject _json = null;
		if (TryGetObject (key, out _json)) {
			nowTryGetKey = "";
			...
			if(logError){
				Debug.LogError("NO [" + nowTryGetKey  + "] in Json : " + Print());
			}
			nowTryGetKey = "";
			return new List<JSONObject>();
		}
	}
	//public List<JSONObject> GetListValue(bool logError = true) {
	//	if (list != null) {
	//		nowTryGetKey = "";
	//		return list;
	//	} else {
	//		if(logError){
	//			Debug.LogError ("[" + nowTryGetKey + "] in Json is not Array");
	//		}
	//	}
	//	nowTryGetKey = "";
	//	return new List<JSONObject>();
	//}
	public List<T> TryGetList<T>(string key,bool logError = true) {
		nowTryGetKey = key;
		JSONObject _json = null;
		if (TryGetObject (key, out _json)) {
			nowTryGetKey = "";
			return _json.GetListValue<T>(logError);
		}else {
			if(logError){
				Debug.LogError("NO [" + key + "] in Json : " + Print());
			}
	...
	public IEnumerable<JSONObject> GetListValue(out int p_listSize, bool logError = true) {
		p_listSize = list.Count;
		return list;
	}
	public List<T> GetListValue<T>(bool logError = true) {
		...
		case "Int32":
			for(f=0; f<len; f++){
				_list.Add((T)(object)(int)list[f].i);
			}
			break;
		case "Int64":
			for(f=0; f<len; f++){
				_list.Add((T)(object)list[f].i);
			}
			break;
		case "Single":
			for(f=0; f<len; f++){
				_list.Add((T)(object)list[f].f);
			}
			break;
		case "Double":
			for(f=0; f<len; f++){
				_list.Add((T)(object)list[f].n);
			}
			break;
		case "Boolean":
			for(f=0; f<len; f++){
				_list.Add((T)(object)list[f].b);
			}
			break;
		case "String":
			for(f=0; f<len; f++){
				_list.Add((T)(object)list[f].GetStringValue("",logError));
			}
			break;
		case "DateTime":
			for(f=0; f<len; f++){
				_list.Add((T)(object)list[f].GetDateTimeValue(default(System.DateTime),logError));
			}
			break;
		case "JSONObject":
			for(f=0; f<len; f++){
				_list.Add((T)(object)list[f]);
			}
			if(logError){
				Debug.LogError("TryGetList<JSONObject>(p_key) Will Work, But You Should Just Use GetListValue(out p_listSize), It's Faster");
			}
			break;
		default:
			if(logError){
				Debug.LogError("JSONObject TryGetList Not Support Type : " + typeof(T).Name + ", Maybe You Can Use IJsonData With TryGetListToData()");
			}
			break;
		}
	...
	public List<T> TryGetListToData<T>(string key,bool logError = true) where T : IJsonData, new(){
		nowTryGetKey = key;
		JSONObject _json = null;
		if (TryGetObject (key, out _json)) {
			nowTryGetKey = "";
			return _json.GetListValueToData<T>(logError);
		}else {
			if(logError){
				Debug.LogError("NO [" + key + "] in Json : " + Print());
			}
	...
	public List<T> GetListValueToData<T>(bool logError = true) where T : IJsonData, new(){
		List<T> _list = new List<T> ();
		T _tempData;
	...
	public Dictionary<KeyT,T> TryGetListToDataDict<KeyT,T>(string key,bool logError = true) where T : IDictJsonData<KeyT>, new(){
		nowTryGetKey = key;
		JSONObject _json = null;
		if (TryGetObject (key, out _json)) {
			nowTryGetKey = "";
			return _json.GetListValueToDataDict<KeyT,T>(logError);
		}else {
			if(logError){
				Debug.LogError("NO [" + key + "] in Json : " + Print());
			}
			nowTryGetKey = "";
			return new Dictionary<KeyT,T>();
		}
	}
	public Dictionary<KeyT,T> GetListValueToDataDict<KeyT,T>(bool logError = true)
	where T : IDictJsonData<KeyT>, new(){
		List<T> _list = GetListValueToData<T> (logError);
		Dictionary<KeyT,T> _dict = new Dictionary<KeyT, T>();
		if(_list == null){
			Debug.LogError("No " + typeof(T).Name + " List In Json : " + Print());
		}else{
			int f;
			int len = _list.Count;
			for(f=0; f<len; f++){
				_dict.Add(_list[f].GetKey(),_list[f]);
			}
		}
	...
	static string nowTryGetKey = "";
	public double TryGetNumber(string key,double defultValue = 0,bool logError = true) {
		nowTryGetKey = key;
		JSONObject _json = new JSONObject();
		if (TryGetObject (key, out _json)) {
			return _json.GetNumberValue(defultValue,logError);
		} else {
			if(logError){
				Debug.LogError("NO [" + key + "] in Json : " + Print());
			}
	...
	public double GetNumberValue(double defultValue = 0,bool logError = true) {
		if (IsNumber) {
			return n;
		} else if (IsString) {
			double _double = 0;
			if (double.TryParse (str, out _double)) {
				if(logError){
					Debug.LogError ("[" + nowTryGetKey + "] in Json is String, Parse to number : " + _double.ToString ());
				}
				return _double;
			} else {
				if(logError){
					Debug.LogError ("[" + nowTryGetKey + "] in Json is String, and can't Parse to number : " + Print());
				}
			}
		} else {
			if(logError){
				Debug.LogError ("[" + nowTryGetKey + "] in Json is not number : " + Print());
			}
		}
		nowTryGetKey = "";
		return defultValue;
	...
	public long TryGetLong(string key,long defultValue = 0,bool logError = true) {
		nowTryGetKey = key;
		JSONObject _json = new JSONObject();
		if (TryGetObject (key, out _json)) {
			return _json.GetLongValue(defultValue,logError);
		} else {
			if(logError){
				Debug.LogError("NO [" + key + "] in Json : " + Print());
			}
	...
	public long GetLongValue(long defultValue = 0,bool logError = true) {
		if (IsNumber) {
			return i;
		} else if (IsString) {
			long _long = 0;
			if (long.TryParse (str, out _long)) {
				if(logError){
					Debug.LogError ("[" + nowTryGetKey + "] in Json is String, Parse to Long : " + _long.ToString ());
				}
				return _long;
			} else {
				if(logError){
					Debug.LogError ("[" + nowTryGetKey + "] in Json is String, and can't Parse to Long : " + Print());
				}
			}
		} else {
			if(logError){
				Debug.LogError ("[" + nowTryGetKey + "] in Json is not Long : " + Print());
			}
		}
		nowTryGetKey = "";
		return defultValue;
	...
	public string TryGetString(string key,string defultValue = "",bool logError = true, ToStringMode p_mode = ToStringMode.ConvertEscapeCharacters) {
		nowTryGetKey = key;
		JSONObject _json = new JSONObject();
		if (TryGetObject (key, out _json)) {
			return _json.GetStringValue(defultValue,logError, p_mode);
		} else {
			if(logError){
				Debug.LogError("NO [" + key + "] in Json : " + Print());
			}
	...
	public string GetStringValue(string defultValue = "",bool logError = true, ToStringMode p_mode = ToStringMode.ConvertEscapeCharacters) {
		string _str = defultValue;
		switch (type) {
		case Type.STRING:
			if (p_mode.FlagIsOn(ToStringMode.DecodeUnicode)) {
				_str = str.DecodeUnicode();
			} else {
				_str = str;
			}
			if (p_mode.FlagIsOn(ToStringMode.ConvertEscapeCharacters)) {
		...
		case Type.NUMBER:
			_str = f.ToString();
			if(logError){
				Debug.LogError ("[" + nowTryGetKey + "] in Json is Number, Parse to string : " + _str);
			}
			break;
		case Type.BOOL:
			_str = b.ToString();
			if(logError){
				Debug.LogError ("[" + nowTryGetKey + "] in Json is Bool, Parse to string : " + _str);
			}
			break;
		case Type.NULL:
			_str = "null";
			if(logError){
				Debug.LogError ("[" + nowTryGetKey + "] in Json is Null, Parse to string : " + _str);
			}
			break;
		case Type.OBJECT:
			_str = ToString();
			if(logError){
				Debug.LogError ("[" + nowTryGetKey + "] in Json is Another Json, Parse to string : " + _str);
			}
			break;
		case Type.ARRAY:
			_str = ArrayExtend.ArrayToString(list.ToArray());
			if(logError){
				Debug.LogError ("[" + nowTryGetKey + "] in Json is Array, Parse to string : " + _str);
			}
			break;
		}
		nowTryGetKey = "";
	...
	public bool TryGetBool(string key,bool defultValue = false,bool logError = true) {
		nowTryGetKey = key;
		JSONObject _json = new JSONObject();
		if (TryGetObject (key, out _json)) {
			return _json.GetBoolValue(defultValue,logError);
		} else {
			if(logError){
				Debug.LogError("NO [" + key + "] in Json : " + Print());
			}
	...
	public bool GetBoolValue(bool defultValue = false,bool logError = true) {
		if (IsBool) {
			return b;
		} else if (IsString) {
			switch (str.ToLower()){
			case "true":
				Debug.LogError ("[" + nowTryGetKey + "] in Json is String, Parse to Bool : True");
				nowTryGetKey = "";
				return true;
				break;
			case "false":
				Debug.LogError ("[" + nowTryGetKey + "] in Json is String, Parse to Bool : False");
				nowTryGetKey = "";
				return false;
				break;
			}
		...
		if(logError){
			Debug.LogError ("[" + nowTryGetKey + "] in Json is not Bool : " + Print());
		}
		nowTryGetKey = "";
		return defultValue;
	}

	public System.DateTime TryGetDateTime(string key,System.DateTime defultValue = default(System.DateTime),bool logError = true) {
		nowTryGetKey = key;
		JSONObject _json = new JSONObject();
		if (TryGetObject (key, out _json)) {
			return _json.GetDateTimeValue(defultValue,logError);
		} else {
			if(logError){
				Debug.LogError("NO [" + key + "] in Json : " + Print());
			}
	...
	public System.DateTime GetDateTimeValue(System.DateTime defultValue = default(System.DateTime),bool logError = true) {
		if (IsString) {
			System.DateTime _dateTime;
			try{
				_dateTime = System.Convert.ToDateTime (str);
				return _dateTime;
			}catch{}
			if(logError){
				Debug.LogError ("[" + nowTryGetKey + "] in Json is String, and can't Parse to DateTime : " + Print());
			}
			nowTryGetKey = "";
			return defultValue;
		}
		...
		if(logError){
			Debug.LogError ("[" + nowTryGetKey + "] in Json is not DateTime String : " + Print());
		}
		nowTryGetKey = "";
		return defultValue;
	}
	...
	public static JSONObject Create() {
		...
		while(result == null && releaseQueue.Count > 0) {
			...
			result.disposeState = DisposeState.Using;
#if DEV
		...
		result.list ??= new List<JSONObject>();
	...
	public static JSONObject Create(System.DateTime val) {
		JSONObject obj = Create();
		obj.type = Type.STRING;
		obj.str = val.ToString();
		obj.str = obj.str.Replace("\r", "").Replace("\\", "\\\\").Replace("\"", "\\\"").Replace("\n", "\\n").Replace("\t", "\\t");
		return obj;
	}
	...
	public static JSONObject Create(ICollection<JSONObject> collection) {
		JSONObject obj = Create();
		obj.type = Type.ARRAY;
		obj.list = new List<JSONObject>();
		foreach (JSONObject addItem in collection) {
			obj.list.Add(addItem);
		}
		return obj;
	}
	public static JSONObject CreateFromData<T>(T data) where T : IJsonData {
		return data.GetJson();
	}
	public static JSONObject CreateFromDatas<T>(ICollection<T> collection) where T : IJsonData {
		JSONObject obj = Create();
		obj.type = Type.ARRAY;
		obj.list = new List<JSONObject>();
		foreach (T addItem in collection) {
			obj.list.Add(addItem.GetJson());
		}
		return obj;
	}
	public static JSONObject Create<T>(ICollection<T> collection, bool logError = true) {
		JSONObject obj = Create();
		obj.type = Type.ARRAY;
		obj.list = new List<JSONObject>();
		switch (typeof(T).Name) {
		case "Int32":
			foreach (T addItem in collection) {
				obj.list.Add(Create((int)(object)addItem));
			}
			break;
		case "Int64":
			foreach (T addItem in collection) {
				obj.list.Add(Create((long)(object)addItem));
			}
			break;
		case "Single":
			foreach (T addItem in collection) {
				obj.list.Add(Create((float)(object)addItem));
			}
			break;
		case "Double":
			foreach (T addItem in collection) {
				obj.list.Add(Create((double)(object)addItem));
			}
			break;
		case "Boolean":
			foreach (T addItem in collection) {
				obj.list.Add(Create((bool)(object)addItem));
			}
			break;
		case "String":
			foreach (T addItem in collection) {
				obj.list.Add(CreateStringObject((string)(object)addItem));
			}
			break;
		case "DateTime":
			foreach (T addItem in collection) {
				obj.list.Add(Create((System.DateTime)(object)addItem));
			}
			break;
		case "JSONObject":
			foreach (T addItem in collection) {
				obj.list.Add(Create((JSONObject)(object)addItem));
			}
			if (logError) {
				Debug.LogError("Create<JSONObject>(collection) Will Work, But You Should Just Use Create(collection), It's Faster");
			}
			break;
		default:
			if (logError) {
				Debug.LogError("JSONObject Create<T>(collection) Not Support Type : " + typeof(T).Name + ", Maybe You Can Use IJsonData With CreateFromDatas<T>(collection)");
			}
			break;
		}
	...
	public void AddField<T>(string name, ICollection<T> val) {
		AddField(name, Create(val));
	}
...
//#endif
	...
	DisposeState disposeState = DisposeState.Using;
	public void Dispose() {
		if (disposeState != DisposeState.Disposed) {
			if (holdTarget != null && (holdTarget.Count > 0)){
				disposeState = DisposeState.WaitDispose;
			} else {
				DoDispose();
			}
		}
	...
	HashSet<object> holdTarget;
	public void HoldByVariable(object p_target, out JSONObject p_variable) {
		p_variable = this;
		holdTarget ??= new HashSet<object>();
		holdTarget.Add(p_target);
	}
	public void ReleaseFromVariable(object p_target) {
		if(holdTarget == null) { return; }

		holdTarget.Remove(p_target);
		if (disposeState == DisposeState.WaitDispose && holdTarget.Count == 0) {
			DoDispose();
		}
	}
}
